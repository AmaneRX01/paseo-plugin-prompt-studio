...
import { canTransitionDraftStatus, draftStatuses, isSendableDraftStatus } from "../shared/draft-lifecycle";
import {
  foldCaseInsensitive,
  normalizeTags,
  renameTagPath,
...
} from "./studio/draft-list-pane";
import {
  planBatchDraftTransitions,
  type BatchDraftAction,
} from "./studio/batch-selection";
import {
  appendBoilerplate,
} from "./studio/boilerplate-preferences";
import {
  selectRecentSnapshots,
  selectVisibleCheckpoints,
  toggleCheckpointStar,
  useHistoryPreferences,
} from "./studio/history-preferences";
import {
  isPathInsideVault,
  projectChoicesFromWorkspaces,
  useRememberedProjectChoices,
  type ProjectChoice,
} from "./studio/project-choices";
import { normalizeNullableFilterSelection } from "./studio/filter-selection";
import {
  createScopeChangeQueue,
  type ScopeChangeQueue,
} from "./studio/scope-change-queue";
import { resolveTagSetResponse } from "./studio/tag-cache";
import { SendPanel, SessionSummary } from "./studio/send-panel";
import { StudioHeader } from "./studio/studio-header";
import { PromptAgentActions } from "./studio/prompt-agent-actions";
import {
  TagChipInput,
  type TagControlLabels,
  type TagSuggestion,
} from "./studio/tag-controls";
import type {
  DraftStatus,
...
import { WorklogView } from "./studio/worklog-view";
import { useWorkspaceDirectory } from "./studio/workspace-directory";
import type { WorkspaceDirectoryEntry } from "./studio/workspace-directory-state";
import {
  Card,
  ConfirmInline,
  Description,
...
export type { StudioProjectContext, StudioTab } from "./studio/studio-types";
    ...
    <View style={{ gap: 12 }}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
        ...
        <View style={{ flex: 1, gap: 2 }}>
          ...
          <MonoMeta theme={theme}>{snapshotId}</MonoMeta>
        </View>
      </View>
      ...
      {query.data ? (
        <View style={{ gap: 10 }}>
          ...
          <MonoMeta theme={theme}>{query.data.snapshot.contentHash}</MonoMeta>
...
function DraftEditor({
  ...
  const deleteMutation = useMutation({ mutationFn: deleteDraft });
  ...
  useEffect(() => {
    const blockState: NavigationBlockState | null = dispatchBusy
      ? "dispatching"
      : lifecycleBusy || tagsBusy || globalMutationBusy
        ? "updating"
      : checkpointBusy
        ? "updating"
  ...
  }, [checkpointBusy, dispatchBusy, draftId, globalMutationBusy, lifecycleBusy, onNavigationStateChange, saveState, scopeBusy, scopePending, tagsBusy]);
  ...
  async function permanentlyDelete() {
    if (!query.data || query.data.draft.summary.status !== "archived"
      || deleteInFlight.current || lifecycleBusy || saveState !== "saved"
      || checkpointBusy || scopePending || scopeBusy || dispatchBusy
      || tagsBusy || globalMutationBusy || generationBusy) return;
  ...
  if (query.isPending) {
    return (
      <View style={{ flex: 1, justifyContent: "center", padding: compact ? 12 : 16 }}>
        <SkeletonRows rows={4} theme={theme} />
      </View>
    );
  }
  ...
  const scopeSelectorDisabled = archived
    || saveState !== "saved"
    || checkpointBusy
    || scopeBusy
    || dispatchBusy
    ...
    || tagsBusy
    || globalMutationBusy;
  const scopeSelectorLabel = selectedScopeProjectId
    ? scopeProjects.find((project) => project.projectId === selectedScopeProjectId)?.projectDisplayName
      ?? selectedScopeProjectId
    : t("editor.scope.inbox");
  ...
  const editorHeader = (
    <View>
      ...
      >
        ...
        />
        <NativeButton
          accessibilityLabel={t("editor.scope.change")}
          disabled={scopeSelectorDisabled}
          label={scopeSelectorLabel}
          onPress={() => setScopePickerOpen((open) => !open)}
          small
          theme={theme}
          variant="outline"
        />
      ...
      {scopePickerOpen || scopePending || saveState !== "saved" || scopeWarning ? (
        <View style={{ gap: 4, paddingBottom: 8, paddingHorizontal: compact ? 0 : 16, paddingTop: 4 }}>
          {scopePickerOpen ? (
            <Card style={{ marginTop: 4 }} theme={theme}>
              <ScrollView style={{ maxHeight: 280 }}>
                <SelectionRow
                  checked={selectedScopeProjectId === null}
                  disabled={scopeSelectorDisabled}
                  kind="radio"
                  onPress={() => {
                  ...
                  theme={theme}
                  title={t("editor.scope.inbox")}
                />
                {scopeProjects.map((project) => (
                  <SelectionRow
                    checked={selectedScopeProjectId === project.projectId}
                    disabled={scopeSelectorDisabled}
                    key={project.projectId}
                    kind="radio"
                    onPress={() => {
                      setScopePickerOpen(false);
                      changeScope(project.projectId);
                    }}
                    theme={theme}
  ...
  const editorBody = (
    <View style={{ gap: 14 }}>
      ...
      <View style={{ alignItems: "center", flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
        ...
        />
        <NativeButton
          label={lifecycleBusy && !deleteMutation.isPending
      ...
      {archived ? (
        <Card danger theme={theme}>
          <ConfirmInline
            ...
            confirmBody={t("editor.delete.body", {
            ...
            confirmTitle={t("editor.delete.title")}
            disabled={saveState !== "saved" || checkpointBusy || scopePending || scopeBusy
              || dispatchBusy || lifecycleBusy || generationBusy || hasPendingDispatch
              || tagsBusy || globalMutationBusy}
            label={deleteMutation.isPending ? t("editor.delete.deleting") : t("editor.delete.action")}
      ...
      <View style={{ borderTopColor: palette.border, borderTopWidth: 1, gap: 6, paddingTop: 10 }}>
        ...
        {visibleSnapshots.map((snapshot) => {
          ...
          return (
            ...
            >
              <Text style={{ color: theme.colors.accent, fontSize: font.caption }}>
                {t("editor.sentVersion", { version: snapshot.version })}
              </Text>
              <MonoMeta style={{ flex: 1 }} theme={theme}>{snapshot.id}</MonoMeta>
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <MonoMeta style={{ flex: 1 }} theme={theme}>{checkpoint.id}</MonoMeta>
                    <Text style={{ color: theme.colors.foregroundMuted, fontSize: font.caption }}>
                  ...
                  <IconButton
                    ...
                    glyph={starred ? "star" : "starOutline"}
...
export function StudioView({
  ...
  const rememberedProjectChoices = useRememberedProjectChoices();
  const projectChoices = useMemo(
    () => {
      const choices = projectChoicesFromWorkspaces(workspaces, emptyProjects);
      const ranks = new Map(rememberedProjectChoices.map((projectId, index) => [projectId, index]));
      return choices.sort((left, right) => (
        (ranks.get(left.projectId) ?? Number.POSITIVE_INFINITY)
        - (ranks.get(right.projectId) ?? Number.POSITIVE_INFINITY)
      ));
    },
    [emptyProjects, rememberedProjectChoices, workspaces],
  );
  useEffect(() => {
