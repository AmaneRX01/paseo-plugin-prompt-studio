...
export type { TagTreeNode } from "../../shared/tags";
...
export function TagTreeDirectory({
  ...
  const directoryMaxHeight = maxHeight ?? 240;
  ...
  function renderManagement(node: TagTreeNode, depth: number, rowBusy: boolean) {
    ...
    if (renaming) {
      ...
      return (
        <View style={{ gap: 4, paddingBottom: 6, paddingLeft: indent, paddingRight: 6 }}>
      ...
      <View key={key}>
        ...
        >
          {node.children.length ? (
            ...
            >
              <Glyph name={expanded ? "collapse" : "expand"} theme={theme} />
            </Pressable>
          ) : (
