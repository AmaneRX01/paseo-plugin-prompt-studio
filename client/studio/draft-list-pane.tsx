...
import type { DraftSummary } from "../../shared/contracts";
import { draftStatuses } from "../../shared/draft-lifecycle";
import { normalizeTags, tagKey, type TagTreeNode } from "../../shared/tags";
...
import { displayTitle, formatWhen, scopeLabel } from "./studio-formatters";
import {
  TagChipInput,
  TagFilterChip,
  TagTreeDirectory,
...
import type { DraftStatus, StudioProjectContext } from "./studio-types";
...
export function DraftListPane({
  ...
  const activeFilterCount =
    (selectedStatusIds.length !== statusIds.length ? 1 : 0)
    + (projectIds !== null ? 1 : 0)
    + (tagPaths?.length ? 1 : 0);
    ...
    return (
      ...
      >
        ...
        <HighlightedText
          numberOfLines={2}
          query={search}
          style={{ color: theme.colors.foregroundMuted, fontSize: font.caption, lineHeight: uiMetrics.compactControlLineHeight }}
          theme={theme}
  ...
  return (
    ...
    >
      <View style={{ borderBottomColor: palette.border, borderBottomWidth: 1, gap: 8, padding: compact ? 12 : 14 }}>
        ...
        <View style={{ alignItems: "center", flexDirection: "row", gap: 8 }}>
          <IconButton
            accessibilityLabel={t(filtersExpanded ? "filter.collapse" : "filter.expand")}
            glyph={filtersExpanded ? "collapse" : "expand"}
      ...
      {pending ? <SkeletonRows rows={4} style={{ paddingHorizontal: 12, paddingVertical: 8 }} theme={theme} /> : null}
      {list}
    </View>
  );
}
