...
import { BoilerplatePicker } from "./boilerplate-picker";
import { GenerationContextSourceCard } from "./generation-context-controls";
import {
  generationStatusMessageKey,
  isUnresolvedGenerationStatus,
} from "./generation-state";
import type { ProjectChoice } from "./project-choices";
import type { SaveState } from "./studio-types";
import type { TagSuggestion } from "./tag-controls";
...
function ContextCounts({
  ...
  return (
    <View style={{ gap: 2 }}>
      <Text style={{ color: theme.colors.foreground, fontSize: font.caption, lineHeight: uiMetrics.compactControlLineHeight }}>
...
function JobStatusCard({
  ...
  job: GenerationJob;
  theme: PluginTheme;
  /** Per-mutation pending flags; each button binds busy/label to its own. */
  pending: {
    sync: boolean;
    resume: boolean;
    apply: boolean;
  ...
  /** True while the unresolved job is being polled in the background. */
  syncing?: boolean;
  onSync: () => void;
  onResume: () => void;
  onApply: () => void;
  ...
  return (
    <Card danger={tone === "danger"} theme={theme}>
      <View style={{ alignItems: "center", flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
        ...
        <Text numberOfLines={1} selectable style={{ color: theme.colors.foregroundMuted, flex: 1, fontSize: font.caption, lineHeight: uiMetrics.compactControlLineHeight }}>
          {job.configuration.provider} / {job.configuration.model}
        </Text>
        <ProtectionNotice protection={job.protection} theme={theme} />
        {syncing ? <ActivityIndicator color={theme.colors.accent} size="small" /> : null}
      </View>
      ...
      {job.status === "conflict" && job.responseMarkdown !== null ? (
        <View style={{ gap: 6 }}>
          ...
          <ScrollView
            ...
            style={{ maxHeight: 240 }}
          >
            <Text selectable style={{ color: theme.colors.foreground, fontSize: font.caption, lineHeight: uiMetrics.controlLineHeight }}>
          ...
          <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
            <NativeButton
              ...
              label={pending.apply ? t("generation.candidate.applying") : t("generation.candidate.apply")}
              onPress={onApply}
              small
              theme={theme}
            />
            <ConfirmInline
              ...
              confirmBody={t("generation.discard.confirmBody")}
              confirmLabel={t("generation.candidate.discard")}
              confirmTitle={t("generation.discard.confirmTitle")}
              disabled={mutationPending}
              label={pending.discard ? t("generation.candidate.discarding") : t("generation.candidate.discard")}
              onConfirm={onDiscard}
              theme={theme}
            />
          </View>
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
          {job.status === "prepared" ? (
            <NativeButton
              busy={pending.resume}
              disabled={mutationPending}
              label={pending.resume ? t("generation.resuming") : t("generation.resume")}
              onPress={onResume}
              small
              theme={theme}
              variant="outline"
          ...
          {canSync ? (
            <NativeButton
              busy={pending.sync}
              disabled={mutationPending}
              label={pending.sync ? t("generation.syncing") : t("generation.sync")}
              onPress={onSync}
              small
              theme={theme}
              variant="outline"
          ...
          {canAbandon ? (
            <ConfirmInline
              ...
              cancelLabel={t("common.cancel")}
              confirmBody={t("generation.abandon.confirmBody")}
              confirmLabel={t("generation.abandon")}
              confirmTitle={t("generation.abandon.confirmTitle")}
              disabled={mutationPending}
              label={pending.abandon ? t("generation.abandoning") : t("generation.abandon")}
              onConfirm={onAbandon}
              theme={theme}
            />
          ) : null}
...
export function PromptAgentActions({
  ...
  const [filters, setFilters] = useState<GenerationContextFilters>(() => initialContextFilters(detail));
  ...
  return (
    <View style={{ gap: 8 }}>
      ...
      {jobQuery.isFetching && !job ? <SkeletonRows rows={2} theme={theme} /> : null}
      {jobQuery.isError ? (
        ...
        <Hint danger theme={theme}>{operationError}</Hint>
      ) : settingsMissing ? (
        <Hint danger theme={theme}>{t("generation.disabled.settings")}</Hint>
      ) : baseDisabledReason ? (
        <Hint theme={theme}>{baseDisabledReason}</Hint>
      ) : null}
      {job ? (
        <JobStatusCard
          job={job}
          ...
          pending={{
            abandon: abandonMutation.isPending,
            apply: applyMutation.isPending,
            discard: discardMutation.isPending,
            resume: resumeMutation.isPending,
            sync: syncMutation.isPending,
          }}
          syncing={jobQuery.isFetching}
          onAbandon={() => {
            setOperationError(null);
      ...
      >
        <ScrollView
          contentContainerStyle={{ gap: 12, paddingBottom: 2, paddingRight: compact ? 2 : 6 }}
        ...
        >
              ...
              <Card theme={theme}>
                <SelectionRow
                  ...
                  kind="checkbox"
                  onPress={() => setAllowProjectRead((current) => !current)}
                  right={previewQuery.data ? (
                    <ProtectionNotice protection={previewQuery.data.preview.protection} theme={theme} />
                  ) : null}
                  subtitle={t("generation.projectRead.help")}
                  theme={theme}
                  title={t("generation.projectRead.label")}
                />
              </Card>
