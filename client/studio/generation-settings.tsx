...
export function GenerationSettingsSection({
  ...
  const dirty = useMemo(() => {
    ...
    if (!editable || !saved) return false;
    const snapshot = editableSettings(saved);
    const sameConfig = (
      left: GenerationProviderConfig | null,
      right: GenerationProviderConfig | null,
    ) => left === right || Boolean(left && right
      && left.provider === right.provider
      && left.model === right.model
      && left.thinkingOptionId === right.thinkingOptionId);
    return !sameConfig(editable.related, snapshot.related)
      || !sameConfig(editable.format, snapshot.format)
      || editable.contextTimeRangeDays.some((value, index) => value !== snapshot.contextTimeRangeDays[index]);
  }, [editable, settingsQuery.data?.settings]);
  const conflict = Boolean(
    updateMutation.error && /conflict|version/i.test(errorMessage(updateMutation.error)),
  );
    ...
    <View style={{ gap: compact ? 8 : 10 }}>
      ...
      {editable ? (
        <>
          ...
          {dirty ? <Hint accent theme={theme}>{t("settings.generation.dirty")}</Hint> : null}
          ...
          {notice ? (
            <Hint accent={!updateMutation.error} danger={Boolean(updateMutation.error)} theme={theme}>
              {notice}
            </Hint>
          ) : null}
        </>
