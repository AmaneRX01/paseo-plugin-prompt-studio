...
Every send starts with a stable logical Dispatch ID and every dispatch has a stable `clientMessageId`. Replaying the same send request reopens its one persisted lineage instead of freezing or delivering a second copy. If retained clients generate different Dispatch IDs, the server still coalesces the same canonical Draft revision and target under a cross-process lock. A failed retry reuses the original snapshot and message ID. When a request has an uncertain result, Prompt Studio reconciles it against the agent timeline before deciding whether another send is safe.
...
Both views are available globally from the Paseo sidebar and from the Command Center with `Ctrl+K` on Windows/Linux or `⌘K` on macOS. They open as global sidebar surfaces, with Workspace-local panel mirrors available inside an open Workspace:
...
In an Agent's message composer, bare `/studio` and `/draft` retain their existing behavior and open Prompt Studio. If either command has non-empty text after it, the plugin creates a new Draft under the current Workspace's Project using that text as Markdown, then opens Prompt Studio; the input is consumed and is not sent to the Agent. `/worklog` is available through the sidebar and Command Center, not as a Slash Command.
...
The settings dialog has two tabs. **General** holds language, descriptions, history limits, and the Prompt Agent settings above. **Migration** lists Project links whose Paseo Project or linked folder no longer exists — the main page stays silent about them. Each listed Project offers **Migrate**, which moves every Draft (including archived ones, keeping checkpoints, events, and history) to an existing Project or Inbox, and **Delete**, a two-step confirmed action that permanently deletes every Draft through the ordinary archive-then-delete path. Per-Draft failures are reported and the link is kept until every Draft is resolved.
...
| Project link | Machine-local relationship between a Paseo Project (identified by its `projectId`) and an external project directory in `project-map.json`; it does not require a Workspace |
...
Drafts created in the full Prompt Studio panel start in the Inbox. Drafts created from a Scratchpad inherit the current Project scope. In Workspace and agent contexts, the Workspace ID only locates the Project. An agent context may also preselect a dispatch target, but a draft never belongs to a Workspace or agent.
...
- If the original RPC response is lost, the client reuses its retained Dispatch ID. The server returns or reconciles that exact lineage and never treats the replay as a fresh send.
...
`catalog.json` is safe to delete. Normal panel loads and window refocuses use the daemon's in-process derived cache instead of repeatedly scanning the vault. Selecting **Refresh files** rebuilds the catalog from draft, event, snapshot, and dispatch files. An open draft is reconciled separately for external edits.
...
If an external project directory becomes unavailable or its Paseo Project is removed, Prompt Studio stays silent on Studio surfaces, preserves the mapping and every draft, and lists the link in Settings → Migration. There the user can migrate its drafts to an existing Project or Inbox, or delete the content permanently. Permanent deletion remains available only through the plugin and is still protected by archive state, version/hash checks, and unresolved-dispatch checks.

The plugin refuses to relink vault subdirectories as external Projects.

The current version does not create a `worklog/` directory.
...
- `client/` may import client and shared modules only.
- `server/` may import server and shared modules only.
- `shared/` must not depend on Node APIs, React, or runtime-specific SDK entries.
- `index.client.tsx` registers the sidebar surfaces, Workspace panels, and commands; `index.server.ts` registers RPCs. Both remain free of business logic.
...
2. The Paseo sidebar shows **Prompt Studio** and **Worklog**, and global Command Center actions open the same surfaces. Prompt Studio exposes draft actions; Worklog exposes only search, refresh, and a read-only timeline.
...
12. With a dead Project link (removed Paseo Project or deleted folder), the main page shows no link warning. Settings → Migration lists it; migration moves every Draft (including archived) to the chosen target and removes the link, deletion removes every Draft and the link, a per-Draft failure keeps the link, and an unavailable migration target is rejected without touching the source.
13. Deleting `catalog.json` still allows a complete list and timeline rebuild.
14. Wide and compact layouts, English and Simplified Chinese, and light and dark themes remain usable. Destructive actions use theme status colors, and confirmations fit compact layouts.
...
- The Paseo plugin API is evolving. After upgrading Paseo or the official `@getpaseo/plugin` / `@getpaseo/client` dependencies, rerun the complete checks and compiler smoke test.
...
- An unavailable external Project directory never deletes its `project-map.json` entry or drafts on its own. Restore the directory to make the link usable again, or use Settings → Migration to migrate or delete the content explicitly.
