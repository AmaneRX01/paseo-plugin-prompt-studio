...
import { z } from "zod";
import { activeDraftStatuses, draftStatuses } from "./draft-lifecycle";
import type { TagTreeNode as SharedTagTreeNode } from "./tags";
...
export const dispatchPendingCountsRpc = defineRpc({
  name: "prompt-studio.dispatch-pending-counts",
  input: z.object({
    projectIds: z.array(
      z.string().trim().min(1).max(512).refine(
        (projectId) => !/[\\/]/.test(projectId),
        "Project IDs must be logical identifiers, not filesystem paths",
      ),
    ).max(200),
  }).strict(),
  output: z.object({
