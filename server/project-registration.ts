import type { ContainerSummary } from "../shared/contracts";
import {
  PromptStudioStore,
  type EnsureContainerResult,
  type ResolvedSourceProject,
} from "./store";
import { normalizePath } from "./storage/filesystem";
...
export async function ensureAndRegisterProjectContainer(
  ...
  if (await store.isManagedPath(source.rootPath)) {
    throw new Error("A Prompt Studio vault subdirectory cannot be linked as an external Project");
  }
