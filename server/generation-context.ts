...
import { defaultGenerationContextFilters } from "../shared/generation";
import { normalizeTags, tagKey, tagsMatchAnyPath } from "../shared/tags";
import { listCheckpointContents } from "./storage/checkpoints";
...
export type { GenerationContextSource } from "../shared/generation";
...
export function buildGenerationContext(input: BuildGenerationContextInput): GenerationContextBuildResult {
  ...
  const targetHistory = input.task === "related" && filters.targetCheckpoints.enabled
...
export async function buildGenerationContextFromStore(
  ...
  const filters = input.task === "related"
    ? input.filters ?? defaultGenerationContextFilters
    : defaultGenerationContextFilters;
  const includeTargetCheckpoints = input.task === "related" && filters.targetCheckpoints.enabled;
  ...
  if (input.task === "related") {
    ...
    for (const summary of catalog.drafts) {
      ...
      const loaded = await loadContextDraft(input.store, summary.id, false);
