...
import { sameDispatchTarget } from "../shared/dispatch";
import {
...
} from "./tag-transactions";
import { buildDraftTimeline } from "./timeline";
import {
  checkpointDocument,
...
} from "./storage/generations";
import { KeyedLockQueue } from "./storage/locking";
import {
  VaultRepository,
...
export class PromptStudioStore {
  ...
  async removeProjectLink(projectId: string): Promise<boolean> {
    await this.initialize();
    return this.withLock("project-map", () => this.vault.removeProjectLink(projectId));
  }

  async listProjectLinks(): Promise<ProjectLink[]> {
    await this.initialize();
    return this.vault.linkEntries();
  }
  ...
  private async pendingDispatchesWhere(
    predicate: (dispatch: Dispatch) => boolean,
  ): Promise<Dispatch[]> {
    return this.withLock("tags-global", async () => {
      const pending: Dispatch[] = [];
      for (const draftId of await this.listDraftIds()) {
        try {
          await this.withLock(draftId, async () => {
            const { containerRoot, draftRoot } = await this.locateDraft(draftId);
            const dispatches = await this.listDispatches(draftRoot, containerRoot);
            pending.push(...dispatches.values.filter(
              (dispatch) => dispatch.status === "pending" && predicate(dispatch),
            ));
          });
        } catch {
          // A draft removed or repaired while this read was in progress is not
          // allowed to prevent healthy pending dispatches from being observed.
        }
      }
      pending.sort((left, right) => right.updatedAt.localeCompare(left.updatedAt));
      return pending;
    });
  }

  async pendingDispatchesForAgent(agentId: string): Promise<Dispatch[]> {
    const normalizedAgentId = agentId.trim();
    if (!normalizedAgentId) return [];
    return this.pendingDispatchesWhere((dispatch) => dispatch.agentId === normalizedAgentId);
  }

  async pendingDispatchesForWorkspace(workspaceId: string): Promise<Dispatch[]> {
    const normalizedWorkspaceId = workspaceId.trim();
    if (!normalizedWorkspaceId) return [];
    return this.pendingDispatchesWhere((dispatch) => (
      dispatch.workspaceId === normalizedWorkspaceId
      || (dispatch.target.kind === "new_agent" && dispatch.target.workspaceId === normalizedWorkspaceId)
    ));
  }

  async pendingDispatchCountsByProject(
    projectIds: readonly string[],
  ): Promise<Record<string, number>> {
    const requested = [...new Set(projectIds.map((projectId) => projectId.trim()).filter(Boolean))];
    const counts = Object.fromEntries(requested.map((projectId) => [projectId, 0]));
    if (!requested.length) return counts;
    const requestedSet = new Set(requested);
    for (const dispatch of await this.pendingDispatchesWhere(() => true)) {
      try {
        const draft = await this.getDraft(dispatch.draftId);
        const projectId = draft.summary.scope.projectId;
        if (projectId && requestedSet.has(projectId)) counts[projectId] += 1;
      } catch {
        // A concurrently deleted or malformed Draft is not a countable
        // project entry and must not hide other healthy pending records.
      }
    }
    return counts;
  }
  ...
  async moveDraftScope(
    ...
    return this.withLock(draftId, async () => {
      ...
      // Ordinary scope edits require an active Draft; the project-migration
      // sweep may move archived Drafts too because scope is metadata-only.
      if (meta.status === "archived" && !options.allowArchived) {
        throw new Error("Restore the archived draft before changing its Scope");
      }
      ...
      await this.writeDraftEvent(source.containerRoot, draftId, {
        ...
        actor: "user",
        details: { sourceContainerId: originalContainerId, targetContainerId, sourceScope, targetScope: scope },
      });
      await this.refreshCatalog();
  ...
  private async readSnapshotUnlocked(draftId: DraftId, snapshotId: string): Promise<Snapshot> {
    const { containerRoot, draftRoot } = await this.locateDraft(draftId);
  ...
  async getSnapshot(draftId: DraftId, snapshotId: string): Promise<Snapshot> {
    return this.withLock(draftId, () => this.readSnapshotUnlocked(draftId, snapshotId));
  }

  async prepareDispatch(
    draftId: DraftId,
    target: DispatchTarget,
    requestedDispatchId: string = this.id("ds"),
  ): Promise<{ snapshot: Snapshot; dispatch: Dispatch; created: boolean }> {
    const dispatchId = dispatchIdSchema.parse(requestedDispatchId);
    return this.withLock(draftId, async () => {
      const { containerRoot, draftRoot } = await this.locateDraft(draftId);
      ...
      if (await exists(dispatchPath)) {
        ...
        if (!sameDispatchTarget(current.target, target)) {
          throw new Error(`Dispatch request ${dispatchId} was already used for a different target`);
        }
        return {
          snapshot: await this.readSnapshotUnlocked(draftId, current.snapshotId),
          dispatch: current,
          created: false,
        };
      }

      // A client-generated Dispatch ID protects RPC replay, but retained UI
      // instances can generate different IDs for the same user intent. Close
      // that gap under the Draft's cross-process lock by comparing canonical
      // revision + target before creating another snapshot or reaching Paseo.
      await this.ensureNoUnresolvedGeneration(draftId, "send this draft");
      const reconciled = await this.reconcileExternalEditUnlocked(
        containerRoot,
        draftRoot,
      ...
      const dispatches = await this.listDispatches(draftRoot, containerRoot);
      if (dispatches.warnings.length > 0) {
        throw new Error("Repair malformed dispatch records before sending this draft");
      }
      const orderedDispatches = [...dispatches.values].sort((left, right) => {
        const priority = { pending: 2, accepted: 1, failed: 0 } as const;
        return priority[right.status] - priority[left.status]
          || right.updatedAt.localeCompare(left.updatedAt);
      });
      for (const current of orderedDispatches) {
        if (current.draftId !== draftId) throw new Error("Dispatch lineage mismatch");
        if (!sameDispatchTarget(current.target, target)) continue;
        const currentSnapshot = await this.readSnapshotUnlocked(draftId, current.snapshotId);
        if (
          currentSnapshot.version === reconciled.meta.version
          && currentSnapshot.contentHash === reconciled.meta.contentHash
        ) {
          return { snapshot: currentSnapshot, dispatch: current, created: false };
        }
      }
      const unresolved = orderedDispatches.find((current) => current.status === "pending");
      if (unresolved) {
        throw new Error(
          `Another dispatch is already in progress for this Draft (${unresolved.id})`,
        );
      }
      ...
      await writeJson(dispatchPath, dispatch, containerRoot);
      await this.writeDraftEvent(containerRoot, draftId, {
      ...
      await this.refreshCatalog();
      return { snapshot, dispatch, created: true };
    });
  }
  ...
  async claimDispatchAttempt(
    draftId: DraftId,
    dispatchId: string,
  ): Promise<{ dispatch: Dispatch; claimed: boolean }> {
    return this.withLock(draftId, async () => {
      ...
      const current = dispatchSchema.parse(await readJson(dispatchPath));
      if (current.draftId !== draftId || current.id !== dispatchId) throw new Error("Dispatch lineage mismatch");
      if (current.status !== "failed") return { dispatch: current, claimed: false };
      const meta = await this.readDraftMeta(draftRoot, containerRoot);
      if (meta.status === "archived") {
      ...
      return { dispatch: dispatchSchema.parse(next), claimed: true };
    });
  }

  async markDispatchAttempt(draftId: DraftId, dispatchId: string): Promise<Dispatch> {
    return (await this.claimDispatchAttempt(draftId, dispatchId)).dispatch;
  }
        ...
        if (await exists(catalogPath)) {
          try {
            ...
            if (!raw || typeof raw !== "object" || (raw as { schemaVersion?: unknown }).schemaVersion !== 1) {
              throw new Error("Expected disposable schema-v1 catalog");
            }
          } catch (error) {
