...
function scopeTimeline(detail: DraftDetail): TimelineEntry[] {
  ...
  for (const event of events) {
    const target = eventScope(event, "targetScope");
...
function statusTimeline(detail: DraftDetail): TimelineEntry[] {
  ...
  const events = detail.events
    ...
    .sort((left, right) => left.at.localeCompare(right.at));
  for (const event of events) {
    const fromStatus = eventDraftStatus(event.details.fromStatus);
    const toStatus = eventDraftStatus(event.details.toStatus);
